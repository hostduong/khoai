...
export async function onRequestPost(context) {
  try {
    ...
    // ==== KIỂM TRA BINDING KV ==== //
    if (!env.KHOAI_KV_LOGGER || typeof env.KHOAI_KV_LOGGER.put !== "function") {
      return new Response(
        JSON.stringify({
          success: false,
          message: "LỖI HỆ THỐNG: KV Logger binding (KHOAI_KV_LOGGER) KHÔNG TỒN TẠI HOẶC KHÔNG HỢP LỆ.",
          error: "env.KHOAI_KV_LOGGER is undefined or invalid binding.",
          suggest: "Hãy kiểm tra lại KV namespace binding trên Cloudflare Pages/Workers và deploy lại production."
        }),
